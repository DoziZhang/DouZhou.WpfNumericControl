...
namespace DouZhou.WpfNumericControl
{
    ...
    /// </remarks>
    public class NumericUpDownUInt32 : NumericUpDownBase, INumericUpDown<uint>
    {
        static NumericUpDownUInt32()
        {
            //重写默认样式
            DefaultStyleKeyProperty.OverrideMetadata(typeof(NumericUpDownUInt32), new FrameworkPropertyMetadata(typeof(NumericUpDownUInt32)));
        }
        #region 基类功能实现
        ...
        {
            //减少按钮执行(鼠标滚轮操作使会溢出）
            var value = Value < uint.MinValue + Increment ? uint.MinValue : Value - Increment;
            if (value < Minimum)
            {
                //小于最小值时，设置为最小值
                Value = Minimum;
        ...
        {
            //增加按钮执行(鼠标滚轮操作使会溢出）
            var value = Value > uint.MaxValue - Increment ? uint.MaxValue : Value + Increment;
            if (value > Maximum)
            {
                //大于最大值时，设置为最大值
                Value = Maximum;
        ...
        private void TryParseTextToValue()
        {
            ...
            //尝试转换成decimal类型
            if (uint.TryParse(sValueString, style, culture, out uint value))
            {
                //转换成功后，强制转换成uint类型
                if (value < Minimum)
                {
        ...
        /// </summary>
        /// <remarks>
        /// uint类型支持的格式化字符串有：G、N0、X、x,D（通用、无小数数字、十六进制、十六进制小写、十进制数）
        /// </remarks>
        ...
        public static readonly DependencyProperty StringFormatProperty =
            DependencyProperty.Register("StringFormat", typeof(string), typeof(NumericUpDownUInt32), new PropertyMetadata("G", OnStringFormatChanged, OnStringFormatCoerceValue));
        ...
        private static void OnStringFormatChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (d is NumericUpDownUInt32 numeric)
            {
        ...
        public static readonly DependencyProperty ValueProperty =
            DependencyProperty.Register("Value", typeof(uint), typeof(NumericUpDownUInt32), new PropertyMetadata((uint)0, OnValueChanged, OnCoerceValue));
        ...
        private static string UpdateValueString(string stringFormat, uint newValue)
        {
            try
            {
                //十六进制、数字、通用格式
        ...
        public static readonly DependencyProperty MinimumProperty =
            DependencyProperty.Register("Minimum", typeof(uint), typeof(NumericUpDownUInt32), new PropertyMetadata(uint.MinValue, OnMinimumChanged, OnMinimumCoerceValue));
        ...
        public static readonly DependencyProperty MaximumProperty =
            DependencyProperty.Register("Maximum", typeof(uint), typeof(NumericUpDownUInt32), new PropertyMetadata(uint.MaxValue, OnMaximumChanged, OnMaximumCoerceValue));
        ...
        public static readonly DependencyProperty IncrementProperty =
            DependencyProperty.Register("Increment", typeof(uint), typeof(NumericUpDownUInt32), new PropertyMetadata((uint)1, OnIncrementChanged, OnIncrementCoerceValue));
        ...
        public static readonly RoutedEvent ValueChangedEvent = EventManager.RegisterRoutedEvent("ValueChanged",
                                                                                                ...
                                                                                                typeof(RoutedPropertyChangedEventHandler<uint>),
                                                                                                typeof(NumericUpDownUInt32));
        /// <summary>
        /// 值改变事件
        /// </summary>
        ...
        {
            RoutedPropertyChangedEventArgs<uint> arg = new RoutedPropertyChangedEventArgs<uint>(oldValue, newValue, ValueChangedEvent);
            RaiseEvent(arg);
        }
        #endregion
